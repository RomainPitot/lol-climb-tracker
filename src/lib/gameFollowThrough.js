 ...
 * elle tient les correctifs actifs (data.corrections, "in_progress" — l'ancien Point de
 * focus en fait partie, comme un correctif sans cible chiffrée, voir lib/corrections.js),
 * et si les alertes du Dashboard changent à cause d'elle. Toujours calculé depuis les vrais
 * chiffres de cette seule game (jamais une moyenne qui dilue) — un correctif porte sur une
 * tendance, donc "pas encore" sur une game isolée n'est jamais un échec en soi, juste un
 * chiffre à situer.
 */
export function buildFollowThroughReport(data, sorted, newGame) {
  ...
  for (const c of data.corrections || []) {
    ...
    if (c.targetValue == null) {
      // Suivi de tendance pur (ex-Point de focus) : pas de cible, donc pas de verdict
      // réussi/pas réussi ici — juste la valeur du jour, à situer soi-même.
      items.push({
        kind: "focus",
        ok: true,
        message: `${c.title} : ${value.toFixed(def.decimals)} sur cette game.`,
      });
      continue;
    }
